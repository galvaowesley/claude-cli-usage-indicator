...
- **Reorder indicators.** `claude-usage-indicator order` asks, and
  `set-order NAME...` applies. `set-order` deliberately refuses to change
  *which* indicators are visible: the names given must be exactly the current
  set, only rearranged, so a reorder can never quietly hide something through
  a typo or a forgotten name. Use `set-indicators` for that instead. The
  interactive picker is forgiving in the other direction: name only the items
  you care about and everything else keeps its current relative position
  behind them, so answering `7` simply moves the seventh item to the front.
- **Change the refresh interval without re-running the installer.**
  `claude-usage-indicator refresh` asks, `set-refresh SECONDS|off` applies,
  and `show` now reports the current value. This was the one installer
  question with no way back short of editing `settings.json` by hand. It is
  also the only subcommand that writes `settings.json` rather than
  `statusline.conf`, since the value is Claude Code's own
  `statusLine.refreshInterval`, which makes it the only one that needs `jq`.
  It merges into the existing `statusLine` object, so the `command`, `type`
  and `padding` keys the installer wrote survive.
- **A way back to defaults.** `claude-usage-indicator defaults` asks which
  scope and confirms first; `set-defaults` applies without prompting.
  Plain `set-defaults` restores what this command manages, every indicator
  visible in the standard order, relative reset times, a 24h clock and the
  refresh interval off, while leaving hand-edited colors, thresholds,
  separator and branch icon untouched. `set-defaults --all` additionally
  restores the whole `statusline.conf`, colors included, copying the old file
  to `statusline.conf.bak` first. The default config is regenerated by
  `statusline.sh` itself rather than duplicated in a second place, so there is
  one source of truth for what "default" means.
...
- **Non-interactive settings API.** `claude-usage-indicator show`,
  `set-indicators NAME...`, `set-order NAME...`, `set-reset FORMAT [CLOCK]`,
  `set-refresh SECONDS|off` and `set-defaults [--all]` read and write the
  settings without prompting, which is what the skill drives and what makes a
  scripted or dotfiles-managed setup possible. Argument order for
  `set-indicators` is display order; every setter rejects an unknown name, an
  empty list or an out-of-range interval rather than writing something the
  status line can't read.
